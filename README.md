...
(This library has been tested with v5.21.0.)
...
```sh
npm install https://github.com/codemonger-io/maplibre-collision-boxes#v0.1.2
```
...
A _developer package_ bears the next release version but followed by a dash (`-`) plus the short commit hash; e.g., `0.1.2-abc1234` where `abc1234` is the short commit hash of the commit used to build the package (_snapshot_).
...
```sh
npm install @codemonger-io/maplibre-collision-boxes@0.1.2-abc1234
```
...
While this library works with `maplibre-gl` version from 5.0.0 through version 5.21.0, you may face a type error at the call of the `collectCollisionBoxesAndFeatures` function if your `maplibre-gl` version is different from the one (5.21.0) for which this library is built.
